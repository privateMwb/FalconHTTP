...
TEST(SerializerOutput, WritesCorrectStatusLine) {
...
TEST(SerializerOutput, IncludesUserSetHeaders) {
...
TEST(SerializerOutput, ComputesContentLengthFromBody) {
    HttpResponse response;
...
TEST(SerializerOutput, AlwaysIncludesConnectionClose) {
    HttpResponse response;
    std::string output = HttpSerializer::serialize(response);
...
TEST(SerializerOutput, DoesNotDuplicateReservedHeaders) {
    HttpResponse response;
    ...
    EXPECT_LE(contentLengthCount, 1u);
