...
TEST(RecoveryThreadSafety, ConcurrentRecoveryProducesIntactLines) {
    ...
    while (pos < output.size()) {
        ...
        EXPECT_TRUE(line.starts_with("[ERROR] GET /thread-"));
        EXPECT_NE(line.find("boom-"), std::string::npos);
    }

    EXPECT_EQ(lineCount, static_cast<std::size_t>(threadCount * callsPerThread));
}
