...
TEST(FileCacheConcurrentAccess, DistinctKeysSurviveConcurrentWrites) {
    ...
    for (int t = 0; t < threadCount; ++t) {
        ...
        ASSERT_TRUE(cache.get("/file-" + std::to_string(t), outEntry));
        EXPECT_EQ(outEntry.content, "content-" + std::to_string(t));
    }
}
...
TEST(FileCacheConcurrentAccess, SharedKeySurvivesConcurrentMixedAccess) {
    constexpr int threadCount = 16;
    ...
    EXPECT_FALSE(sawTornValue.load());
}
