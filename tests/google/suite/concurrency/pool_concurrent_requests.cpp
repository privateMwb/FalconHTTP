...
TEST(PoolConcurrentRequests, ConcurrentDispatchHasNoCrossRequestLeakage) {
    ...
    EXPECT_FALSE(sawMismatch.load());
    EXPECT_EQ(handlerInvocations.load(), threadCount * requestsPerThread);
}
...
TEST(PoolConcurrentRequests, ConcurrentDispatchResolvesCorrectMethodUnderContention) {
    Router router;
    ...
    EXPECT_FALSE(sawWrongStatus.load());
    EXPECT_EQ(getCount.load(), 4 * iterationsPerThread);
    EXPECT_EQ(postCount.load(), 4 * iterationsPerThread);
}
