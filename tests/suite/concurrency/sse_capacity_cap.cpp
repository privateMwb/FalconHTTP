...
static void concurrent_streams_beyond_cap_get_503() {
    ...
    CHK(firstPreamble.starts_with("HTTP/1.1 200 OK\r\n"));
    CHK(firstPreamble.find("text/event-stream") != std::string::npos);
    ...
    CHK(secondResponse.starts_with("HTTP/1.1 503 Service Unavailable\r\n"));
    closeSocket(second);
    ...
    CHK(thirdPreamble.starts_with("HTTP/1.1 200 OK\r\n"));
