...
// README) - this uses BENCH_SOLO() with a single lambda per tier, timing
// FalconHTTP alone. Unlike Core/Access benchmarks, each tier here is a
// distinct BENCH_SOLO() call with its own fixed route-table size - the
// growth is in the table itself, not in the SMALL/MEDIUM/LARGE
// iteration count each BENCH_SOLO() call already runs internally.
//
// Covers:
