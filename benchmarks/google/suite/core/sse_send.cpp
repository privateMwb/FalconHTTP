...
// continuously-drained peer for the full run - a send() into a socket
// no one reads from would fill the kernel send buffer within a few
// thousand iterations and then block forever. A background thread on
// the client side of a real loopback connection discards everything
// as fast as it arrives for exactly this reason.
//
