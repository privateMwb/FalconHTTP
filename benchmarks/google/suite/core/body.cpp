...
static void parse_body_heavy(benchmark::State& state) {
    for (auto _ : state) {
        HttpRequest request = HttpParser::parse(kRawRequest);
        benchmark::DoNotOptimize(request);
    }
}
BENCHMARK(parse_body_heavy);

// Measures serialize() on a response with a 64 KiB body.
static void serialize_body_heavy(benchmark::State& state) {
...
BENCHMARK(serialize_body_heavy);
