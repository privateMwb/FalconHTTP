...
`packaging/recipes/falconhttp/all/conanfile.py`). Scoped to what isn't obvious
...
- **The release tarball extracts to `FalconHTTP-<version>`, not
  `FalconHTTP-<version>`** — the GitHub repo is `FalconHTTP`; the CMake
  project/library target inside it is `FalconHTTP`. `cd` into whatever
...
```bash
find $HOME/staging/lib/cmake/FalconHTTP -name "*.cmake"
```

Expect `FalconHTTPConfig.cmake`, `FalconHTTPConfigVersion.cmake`,
`FalconHTTPTargets.cmake` (plus a per-config
`FalconHTTPTargets-<config>.cmake`) under `lib/cmake/FalconHTTP/`.
...
target_link_libraries(test PRIVATE FalconHTTP::FalconHTTP)
EOF
...
int main() {
    ...
    router.get("/health", [](const rain::HTTP::HttpRequest&, rain::HTTP::HttpResponse&) {});
    return 0;
}
EOF
...
constructs a `rain::Routing::Router` and registers a route (`Router::get()`
is compiled into the library, not header-inline, so this actually
exercises the link step) and returns, so success already confirms the
header parsed, the library linked, and the symbols resolved.
...
```bash
conan create packaging/recipes/falconhttp/all --version 1.0.0 \
...
`packaging/recipes/falconhttp/all/test_package/` against the freshly-built
package — a `FalconHTTP linked and constructed successfully.` line in
