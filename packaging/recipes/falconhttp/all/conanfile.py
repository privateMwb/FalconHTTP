...
class Conan(ConanFile):
    ...
    # edited here.
    name = "falconhttp"
    cmake_name = "FalconHTTP"  # matches project()'s name in the top-level CMakeLists.txt
    version = "1.0.0"

    url = "https://github.com/privateMwb/FalconHTTP"
    description = "Embedded C++ HTTP server library built on this project's own container, caching, concurrency, JSON, and function-wrapper libraries."
    topics = (
        "http",
        "http-server",
        "web-framework",
    ...
    # static-library: src/FalconHTTP/*.cpp compiles into libFalconHTTP.a, so
    ...
    def source(self):
        ...
        # no .git directory and no submodule content -- FalconHTTP's own
        # internal libraries (FunctionPro, HashMapPro, JsonParser,
        # LRUCache, ThreadPoolPro, VectorPro) live under libs/internal/ as
